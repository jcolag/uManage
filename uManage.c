...
void handle_alarm (int sig) {
    ...
    for(idx = 0; idx < opts.menu_len; idx++) {
        if (opts.userdef[idx] == 1) {
            ...
            if (opts.use_database) {
                write_duration_to_database(current.csv, opts.menu_items[idx], 1);
            }
            opts.userdef[idx] = 0;
        }
    }
    ...
    if (current.jiggle_since == 0 && opts.jiggle != 0) {
        ...
        if (opts.use_database) {
            write_keepalive_to_database(current.csv, 1);
        }
    }
    ...
    if (current.pause_since != 0 && opts.pause == 0) {
        ...
        if (opts.use_database) {
            write_duration_to_database(current.csv, "Pause", 1);
        }
    }
#endif
    ...
    if(is_window_updated(&current, &poll_continue, opts.use_database)) {
        if (opts.use_database) {
            write_activity_to_database(current.csv, 1);
        }
        if (opts.text_out) {
...
void *get_weather_data(void *arg) {
    ...
    while (wxLinesRemaining()) {
        nextWxLine(csv);
        write_weather_to_database(csv, 1);
    }
    thr_wx_running = -1;
    return NULL;
}
